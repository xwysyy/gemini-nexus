...
Outputs a versioned ZIP under `release/` (e.g. `release/gemini-nexus-v4.2.7.zip`), with `manifest.json` at the ZIP root.
...
1. `Build & Release (Chrome Extension)` (`.github/workflows/release.yml`)
   - On tag push (e.g. `v4.2.7`): builds and uploads the ZIP to GitHub Releases.
